...
import { getTrendingStickers, searchStickers } from '../services/klipyService';
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const data = query.trim()
        ? await searchStickers(query.trim(), 30)
        : await getTrendingStickers(30);
      if (!cancelled) {
        setItems(data);
        setIsLoading(false);
        if (!data.length && !query.trim()) {
          setError('No stickers available right now.');
        }
      }
    };
  ...
  const content = (
    <div className="fixed inset-0 z-[9998] flex flex-col bg-black/60 backdrop-blur-sm">
      <div className="bg-[#0b141a] text-white px-4 py-3 flex items-center justify-between shadow-md">
        <span className="font-bold text-sm sm:text-base">Stickers</span>
        <button
          onClick={onClose}
          ...
          {items.map((item) => (
            ...
            >
              <img
                src={item.previewUrl}
                alt="Sticker"
                className="w-full h-24 object-contain bg-slate-900 group-hover:scale-105 transition-transform duration-200"
                loading="lazy"
              />
            </button>
          ))}
